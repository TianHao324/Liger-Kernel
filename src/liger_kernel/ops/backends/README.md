...
backends/
├── README.md          
├── __init__.py         # Imports all vendor packages to trigger registration
...
register_vendor(
    VendorInfo(
        vendor="<vendor>",
        device="<device>",
        module_path="liger_kernel.ops.backends._<vendor>.ops",
    )
)
```

### Step 3: Import Your Vendor Package

Add import in `backends/__init__.py` to trigger registration:

```python
from liger_kernel.ops.backends import _<vendor>  # noqa: F401
```

### Step 4: Ensure Device Detection Works
...
### Step 5: Implement Vendor-Specific Operators
...
### Step 6: Export in `ops/__init__.py`
